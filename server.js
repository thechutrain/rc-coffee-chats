...
const run = async () => {  
  ...
  const todaysActiveEmails = await getEmailsForDay({ emails: activeEmails, userConfigs, day: today.getDay() });
  ...
  const emailsToMatch = todaysActiveEmails.filter(email => ! noEmailToday.includes(email) );
...
const runWarningMessages = async () => {  
  ...
  let emailsForTomorrow = await getEmailsForDay({ emails: subscribedEmails, userConfigs, day: tomorrow.getDay() });
  logger.info('emailsForTomorrow', emailsForTomorrow);
  
  const warningsExceptions = await getEmailExceptions({tableName: "warningsExceptions"});
  logger.info('warningsExceptions', warningsExceptions);
  
  const noEmailTomorrow = await getEmailExceptions({tableName: "noNextMatch"});
  logger.info('noEmailTomorrow', noEmailTomorrow);
  
  const warningMessageEmails = emailsForTomorrow.filter(email => !(warningsExceptions.includes(email) || noEmailTomorrow.includes(email)) );
...
const handlePrivateMessageToBot = async (body) => {
  ...
  if (message == 'warnings off') {
    db.serialize(function() {
      db.run('INSERT OR REPLACE INTO warningsExceptions(email) VALUES (?)', fromEmail);
    });
    zulipAPI.messages.send({
      to: fromEmail,
      type: "private",
...
const testDB = () => {
  db.all('SELECT * FROM matches WHERE email1 = "test@test.com" OR email2="test@test.com"', (err, rows) => {logger.info(rows)})
}
const testMatches = async () => {
  ...
  const todaysActiveEmails = await getEmailsForDay({ emails: activeEmails, day: (new Date()).getDay() });
